...
class DataLoader(ABC):
    ...
    @abstractmethod
    def __init__(self, directory, manifest, max_buffer, run_length, evaluation, sample_length=True, use_lengths=True, reserved_users=False):

        # Open dataset files
        self.directory = directory
        with open(directory + "/" + manifest) as infile:
            raw = [line.split('\t') for line in infile.read().split('\n') if len(line)]
            if not evaluation:
                for line in raw:
                    self.shuffle(line[0])
            self.files = {line[0]: {"weight": int(line[1]), "fd": open(directory + "/" + line[0]) if exists(directory + "/" + line[0]) else None, "line_buffer": [], "eof": False} for line in raw}

        if reserved_users:
            self.files['reserved'] = {"weight": 0, "fd": open(directory + "/" + "reserved"), "line_buffer": [], "eof": False}
    ...
    def shuffle(self, filename=None):
        ...
        for filename in filenames:
            ...
            output_stream = subprocess.Popen('shuf ' + path + ' > ' + path + "_shuffled && mv " + path + '_shuffled ' + path, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def transform_vector(self, vec):
        ...
        mask = torch.cat( ( torch.zeros(M+1), torch.ones(size-M) ) ,dim = 0).bool()
        if not self.ul:
...
class KeystrokeDataLoader(DataLoader):
    ...
    @abstractmethod
    def __init__(self, directory, manifest, max_buffer, evaluation, run_length, include_others, sample_length=True, use_lengths=True, reserved_users=False):
        super().__init__(directory, manifest, max_buffer, run_length, evaluation, sample_length, use_lengths, reserved_users)
        for cl in self.files:
            self.files[cl]["session_buffer"] = {'class': None, 'st': 0, 'buffer': []}
    ...
    def user_from_tensor(self, t):
        return [self.inv_mapping[i] for i in torch.argmax(t, dim=1).data.cpu().numpy()]
        ...
        if self.eval:
            while len(file['session_buffer']['buffer']) < self.tuple_size:
                if len(file['line_buffer']) > 0:
                    ...
                    try:
                        src_ip, dst_ip = self.process_ip(raw[2]), self.process_ip(raw[3])
                    except:
                    ...
                    for i in range(vlen):
                        ...
                        pos.append(cum_len)
                        cum_len += len(sot) / 512
                    file['session_buffer'] = {'class': real_class, 'st': timestamp, 'buffer': vecs, 'src_ip': [src_ip for _ in range(vlen)], 'dst_ip': [dst_ip for _ in range(vlen)], 'pos': pos}
                else:
                    ...
                    if not file['line_buffer'] or not len(file['line_buffer']):
                        ...
                        file['fd'].close()
                        if already_reloaded:
                            print(f"Warning - Class {cl} does not contain enough elements for tuple size {self.tuple_size}")
                            return False
                        if not continuous:
                            return False
                        self.refresh([cl])
            ...
            while not len(file['session_buffer']['buffer']):
                ...
                if not raw or not len(raw):
                    ...
                    parsed = [item.strip().split('\t') for item in raw if len(item)]
                    try:
                        src_ips, dst_ips = [self.process_ip(item[2]) for item in parsed], [self.process_ip(item[3]) for item in parsed]
                    except:
                        src_ips, dst_ips = [None for item in parsed], [None for item in parsed]
                    pos = [float(item[-1]) for item in parsed]
                    sols = [self.parse_lengths(item[5].split(',')) for item in parsed]
                    sots = [[float(t) for t in item[4].split(',')] for item in parsed]
                    vecs = [torch.tensor([sols[i], sots[i]]) for i in range(len(sols))]
                    file['session_buffer'] = {'class': parsed[0][0], 'st': 0.0, 'buffer': vecs, 'src_ip': src_ips, 'dst_ip': dst_ips, 'pos': pos}
        return True
    ...
    def get_item(self,cl,pop=True,non_pop_offset=0,ips=False):
        if not pop and self.eval:
            pop=True
        if ips and pop:
            return self.files[cl]['session_buffer']['buffer'].pop(0), self.files[cl]['session_buffer']['src_ip'].pop(0), self.files[cl]['session_buffer']['dst_ip'].pop(0), self.files[cl]['session_buffer']['pos'].pop(0)
        elif ips and not pop:
            return self.files[cl]['session_buffer']['buffer'][non_pop_offset], self.files[cl]['session_buffer']['src_ip'][non_pop_offset], self.files[cl]['session_buffer']['dst_ip'][non_pop_offset], self.files[cl]['session_buffer']['pos'][non_pop_offset]
        elif not ips and pop:
            return self.files[cl]['session_buffer']['buffer'].pop(0), self.files[cl]['session_buffer']['pos'].pop(0)
        else:
            return self.files[cl]['session_buffer']['buffer'][non_pop_offset], self.files[cl]['session_buffer']['pos'][non_pop_offset]


class ClassificationKeystrokeDataLoader(KeystrokeDataLoader):

    def __init__(self, directory, max_buffer=100000000, evaluation=False, run_length=7, include_others=False, use_ips=False):
        super().__init__(directory, "classification_manifest", max_buffer, evaluation, run_length, include_others)
        self.reset_choice()
        self.uip = use_ips
    ...
    def sample(self):
        ...
        return {"meta": {"positions": positions, "real_class": file['session_buffer']['class'], "st": file['session_buffer']['st']}, "data": ts, "masks": masks, "label": lbl, "src": src, "dst": dst, "offsets": offsets} 
    ...
    def weights(self, device="cpu"):
        ...
        weights = torch.tensor([total_weight / w if w > 0 else 0 for w in weights]).float()
        weights *= len(weights) / weights.sum().item()
        weights = weights.to(device)
        return weights

    def user_count(self):
        return len([key for key in self.class_weights if key != "reserved"]) if self.include_others else len([key for key in self.class_weights if key != "others" and key != "reserved"])


class AuthenticationKeystrokeDataLoader(KeystrokeDataLoader):

    def __init__(self, directory, max_buffer=10000000, evaluation=False, run_length=7, sample_length=True, use_lengths=True, use_ips=False, reserved_users=False):
        super().__init__(directory, "authentication_manifest", max_buffer, evaluation, run_length, False, sample_length, use_lengths, reserved_users)
        self.user_weights = {c: self.class_weights[c] for c in self.class_weights if c != "others" and c != "reserved"}
    ...
    def reset_choice(self):
        ...
        for cl in self.user_weights:
            if self.files[cl]['fd'] is None:
                continue
            if not self.ru:
                other_classes = random.choices([c for c in self.class_weights.keys() if c != cl and c != "reserved" and self.files[cl]['fd'] is not None], k=wpc)
                self.choice_queue += [(cl_data, cl) for cl_data in other_classes]
            else:
    ...
    def sample(self, others=False):
        ...
        for tidx in range(self.tuple_size):
            if self.uip:
                raw, src, dst, p = self.get_item(data_cl,pop=pos or data_cl == "others" or data_cl == "reserved",non_pop_offset=tidx, ips=self.uip)
                vec, mask, position = self.transform_vector(raw)
            else:
                raw, p = self.get_item(data_cl,pop=pos or data_cl == "others" or data_cl == "reserved",non_pop_offset=tidx)
                vec, mask, position = self.transform_vector(raw)
            samples.append(vec)
            masks.append(mask)
            positions.append(position)
        ...
        return {"meta": {"positions": positions, "real_class": data_cl, "st": file['session_buffer']['st']}, "data": ts, "masks": masks, "user": user_lbl, "label": lbl, "src": src, "dst": dst, "offsets": offsets}
    ...
    def get_batch(self, batch_size=64, others=False, device="cpu"):
        ...
        return True, batch, labels, masks, users, meta, offsets, src_ips, dst_ips
...
class AlternateAuthenticationKeystrokeDataLoader(AuthenticationKeystrokeDataLoader):

    def __init__(self, directory, max_buffer=10000000, evaluation=False, max_run_length=32, min_run_length=16, dropout=1.0, sample_length=True,  use_lengths=True, use_ips=False, reserved_users=False):
        super().__init__(directory, max_buffer, evaluation, max_run_length, sample_length,  use_lengths, use_ips, reserved_users)
    ...
    def get_batch(self, batch_size=64, others=False, device="cpu", strict=True):
        while True:
            flag, batch, labels, masks, users, meta, offsets, src_ips, dst_ips = super().get_batch(batch_size, others, device)

            dropout = self.dropout
            if (self.eval and dropout < 0) or (not self.eval and dropout == 0.0) or (not flag):
                return flag, batch, labels, masks, users, meta, offsets, src_ips, dst_ips

            positions = [m["positions"] for m in meta]
            N, _, _, D = batch.shape
            # dropout -- remove end of vectors. Deterministic for eval, random for training
            if self.eval:
                trunc_len = self.run_length - math.floor(dropout * (self.run_length - self.min_run_length))
                mask = torch.cat( (torch.zeros(N, trunc_len+1).bool(), torch.ones(N, self.run_length - trunc_len).bool()), dim=1 ).to(device).unsqueeze(1)
                if strict:
                    metamask = torch.logical_and(torch.logical_not(mask), masks).any(dim=2).squeeze()
                    if metamask.all():
                        continue
                    ...
                    meta = [meta[i] for i in range(metamask.size(0)) if metamask[i].item()]
                else:
                    masks |= mask
            elif False:
                max_trunc_len = self.run_length - math.floor(dropout * (self.run_length - self.min_run_length))
                indices = torch.randint(self.run_length-max_trunc_len, self.run_length+1, (N,)).to(device)
                mask = torch.zeros(N,self.run_length+1).to(device)
                mask[torch.arange(N), indices] = 1
                mask = mask[:,:-1]
                mask = torch.cat( (torch.zeros(N,1).to(device), torch.cumsum(mask, dim=1).bool().to(device) ), dim=1)
                masks |= mask.unsqueeze(1).bool()
            else:
                max_trunc_len = self.run_length - math.floor(dropout * (self.run_length - self.min_run_length))
                new_masks = torch.zeros_like(masks).bool()
                for b in range(new_masks.size(0)):
                    trunc_len = random.randint(max_trunc_len, self.run_length+1)
    ...
    def typenetbatch(self, batch_size=64, others=False, device="cpu", strict=True):
        flag, batch, labels, masks, users, meta, offsets, src_ips, dst_ips = self.get_batch(batch_size, others, device, strict)
        if not flag:
        ...
        for i in range(masks.shape[0]):
            ...
            #print(start_idx)
            #print(end_idx)
            batch[i,:,:end_idx-start_idx+1] = batch[i,:,start_idx:end_idx+1]
...
class AlternateClassificationKeystrokeDataLoader(ClassificationKeystrokeDataLoader):

    def __init__(self, directory, max_buffer=10000000, evaluation=False, max_run_length=32, min_run_length=16, dropout=1.0, use_ips=False, include_others=False):
        super().__init__(directory, max_buffer, evaluation, max_run_length, include_others, use_ips)
        self.min_run_length = min_run_length
        self.dropout = dropout
    ...
    def get_batch(self, batch_size=64, device="cpu", strict=True):
        while True:
            flag, batch, labels, masks, _, meta, offsets, src_ips, dst_ips = super().get_batch(batch_size, device)

            dropout = self.dropout
            if (self.eval and dropout < 0) or (not self.eval and dropout == 0.0) or (not flag):
                return flag, batch, labels, masks, None, meta, offsets, src_ips, dst_ips
