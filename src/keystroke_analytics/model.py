...
import math
from torch.nn import TransformerEncoder, TransformerEncoderLayer, MultiheadAttention, Parameter
...
class ClassificationTransformer(nn.Module):
    ...
    def __init__(self, n_users, d_in, n_head, n_layers, dropout = 0.1, normalizer="tan", partition=False, partition_count=4, partition_size=8, d_pos=-1, use_length=True, full=False):
        super().__init__()
        ...
        if type(normalizer) is str:
            self.norm = nn.ModuleList([setup_normalizer(normalizer) for _ in range(d_in)])
        else:
            self.norm = []
        ...
        self.d_part = 0
        if partition:
            self.bins = nn.ModuleList([LearnablePartition(self.norm[i], bin_count=partition_size) for i in range(self.d_in) for _ in range(self.partition_count)])
        ...
        if d_pos == -1:
            self.d_pos = (1<<(self.d_in + self.d_ip + self.d_part - 1).bit_length()) - (self.d_in + self.d_part + self.d_ip)
            if self.d_pos % 2:
                self.d_pos += 1
        ...
        # Transformer Layer
        self.pos_encoder = PositionalEncoding(self.d_pos, self.d_model - self.d_pos, dropout)
        encoder_layers = TransformerEncoderLayer(self.d_model, n_head, dropout=dropout)
        ...
        self.class_embedding = nn.Parameter(torch.rand(self.d_model - self.d_ip))
    ...
    def forward(self, session, mask, offsets, src_ip = None, dst_ip = None):
        ...
        if self.d_part > 0:
            for dim in range(self.d_in):
                session = torch.cat((session, torch.cat(tuple(self.bins[i](session[:,:,dim].unsqueeze(2)) for i in range(dim*self.pc, (dim+1)*self.pc)), dim=2)), dim=2)

        # Add position encoding and user encoding dimensions
        session = torch.cat((session, torch.zeros((N*K, S, self.d_ip + self.d_pos)).to(session.device)), dim=2)

        # Add class embedding
        class_embed = self.class_embedding.unsqueeze(0).expand(N*K,self.d_model-self.d_ip)
        if self.d_ip:
...
class AuthenticationTransformer(nn.Module):
    ...
    def __init__(self, n_users, d_in, n_head, n_layers, dropout = 0.1, normalizer="tan", partition=False, partition_count=4, partition_size=8, d_pos=-1, d_user=16, use_length=True, full=False, embed_lengths=False):
        super().__init__()
        self.model_type = 'Transformer'
        self.partition_count = partition_count
        ...
        # Length embedding
        if self.el:
            self.length_embedding = nn.Linear(1500, self.partition_count*partition_size)
        ...
        if d_pos == -1:
            self.d_pos = (1<<(self.d_in + self.d_ip + self.d_user + self.d_part - 1).bit_length()) - (self.d_in + self.d_part + self.d_user + self.d_ip)
            if self.d_pos % 2:
                self.d_pos -= 1
                self.d_user += 1
            print(f"Set pos dim to {self.d_pos}")

        self.d_model = self.d_in + self.d_pos + self.d_part + self.d_user + self.d_ip
        ...
        # Classification token
        self.class_embedding = nn.Parameter(torch.rand(self.d_model - (self.d_user + self.d_ip)))
        self.class_embedding.requiresGrad = True
    ...
    def forward(self, session, mask, offsets, users, src_ip = None, dst_ip = None):
        ...
        if self.el:
            ...
            L_in[torch.arange(N*S), session.reshape(N*S, -1)[:, 0].long()] = 1.0
        ...
        if self.el:
            session = torch.cat((session, self.length_embedding(L_in.reshape(N*S, 1500)).reshape(N,S,self.d_part//2)), dim=2)

        # Add position encoding and user encoding dimensions
        session = torch.cat((session, torch.zeros((N*K, S, self.d_user + self.d_ip + self.d_pos)).to(session.device)), dim=2)

        # Add class embedding
        class_embed = self.class_embedding.unsqueeze(0).expand(N*K,self.d_model - (self.d_user + self.d_ip))
        session[:, 0, :self.d_in+self.d_part] += class_embed[:,:self.d_in+self.d_part]
        session[:, 0, -self.d_pos:] += class_embed[:,-self.d_pos:]

        # Add IP embedding
        if self.d_ip:
            if src_ip is not None:
                session[:, 0, self.d_in+self.d_part:self.d_in+self.d_part+self.d_ip//2] += src_ip
            if dst_ip is not None:
                session[:, 0, self.d_in+self.d_part+self.d_ip//2:self.d_in+self.d_part+self.d_ip] += dst_ip

        # Add user embedding
        session = session + torch.cat((torch.zeros(N*K,S,self.d_in + self.d_part + self.d_ip).to(session.device), self.embedding(users).unsqueeze(1).expand(N*K,S,self.d_user), torch.zeros(N*K,S,self.d_pos).to(session.device)), dim=2)
...
class LearnablePartition(nn.Module):
    def __init__(self, normalizer, bin_count=8, max_value=1.5, negative=False, offset_init=0):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if not self.negative:
            ...
            mean = torch.cat((self.m, val), dim=0).unsqueeze(0).unsqueeze(0).expand((N,S,self.n))
        else:
            val = -torch.flip(torch.cumsum(torch.flip(s * self.f, (0,)), dim=0)[:-1], (0,)) - self.mn
            mean = torch.cat((val, -self.m), dim=0).unsqueeze(0).unsqueeze(0).expand((N,S,self.n))
    ...
    def build_bins(self):
        ...
        bins = [(mean[i]-s[i], mean[i], mean[i]+s[i]) for i in range(mean.shape[0])]
        bins = [(self._to_pkt_size(b[0]), self._to_pkt_size(b[1]), self._to_pkt_size(b[2])) for b in bins]
        return bins
...
class LogNormalizer(nn.Module):
    ...
    def fi(self, x):
        if not torch.is_tensor(x):
            x = torch.tensor(x)
        return torch.sign(x) * (torch.exp(torch.abs(x) * np.log(self.max)) - 1.0)
...
def ROC(model, data_points, targets, masks, positions, users):
    ...
    with torch.no_grad():
        for batch in range(len(data_points)):
            tdata, target, mask, pos = data_points[batch], targets[batch], masks[batch], positions[batch]
            if results is None:
                results = F.softmax(model(tdata, mask, pos, users[batch]), dim=1)[:,1]
            else:
                results = torch.cat((results, F.softmax(model(tdata, mask, pos, users[batch]), dim=1)[:,1]), dim=0)
            real.extend(torch.argmax(target.squeeze(), dim=1).data.cpu().numpy())
...
class FSNet(nn.Module):
    ...
    def __init__(self, n_users, d_model=128, dropout=0.1):
        ...
        # Encoders
        self.lenc = nn.GRU(self.d_model, self.d_model, 2, dropout=dropout, bidirectional=True)
        self.tenc = nn.GRU(self.d_model, self.d_model, 2, dropout=dropout, bidirectional=True)

        # Decoders
        self.ldec = nn.GRU(4*self.d_model, self.d_model, 2, dropout=dropout, bidirectional=True)
        self.tdec = nn.GRU(4*self.d_model, self.d_model, 2, dropout=dropout, bidirectional=True)
    ...
    def forward(self, x, users):
        ...
        # Dense Layer
        DL_in = torch.cat((LZE, LZD, LZE*LZD, torch.abs(LZE-LZD), TZE, TZD, TZE*TZD, torch.abs(TZE-TZD), U), dim=1)
...
class TypeNet(nn.Module):
    ...
    def forward(self, session, users, positions):
        ...
        session = torch.cat((session, self.user_embedding(users).unsqueeze(1).expand(N*K,S,self.d_model-2)), dim=2)
        session = self.bn1(session.permute(0,2,1)).permute(0,2,1) 

        # Pack sequence
        session = nn.utils.rnn.pack_padded_sequence(session, positions, batch_first=True, enforce_sorted=False)
