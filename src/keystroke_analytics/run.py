...
def train(
    ...
    while True:
        ...
        if batch % log_interval == 0 and batch > 0:
            ...
            print(
                f"| epoch {epoch:3d} | batch {batch:5d} / {total_batch_count:5d} | "
...
def evaluate(
    ...
    with torch.no_grad():
        while True:
            ...
            if FSNet:
                ...
                if users is not None and src_ips is not None:
                    output = model(
                        data, masks, offsets, users, src_ips, dst_ips
                    )
                    local_y_pred = model.prediction(output, users)
                elif users is not None and src_ips is None:
                    output = model(data, masks, offsets, users)
            ...
            y_true.extend(
                torch.argmax(target.reshape(output.size(0), -1), dim=1)
                .data.cpu()
                .numpy()
            )
            ...
            if users is not None:
                ...
                if ROC or set_thresholds >= 0:
                    ...
                    real = torch.argmax(
                        target.reshape(output.size(0), -1), dim=1
                    )
                ...
                if cm_per_user is None:
                    cm_per_user = torch.tensor(
                        [[[0.0, 0.0], [0.0, 0.0]] for _ in range(model.n_users)]
                    ).to(device)
    ...
    cf_matrix = confusion_matrix(
        y_true_real,
        y_pred_real,
        labels=[str(t) for t in eval_data.sorted_users],
    )
    ...
    if auth:
        ...
        tfp = torch.tensor([[0.0, 0.0] for _ in range(len(cf_matrix))]).to(
            device
        )
        for u in range(len(cf_matrix)):
            if not cf_matrix[u, u]:
                continue
            tp = cf_matrix[u, u] / sum(
                cf_matrix[u, i] for i in range(len(cf_matrix))
            )
            fp = 1 - (
                cf_matrix[u, u]
                / sum(cf_matrix[i, u] for i in range(len(cf_matrix)))
            )
            tfp[u, 0] = fp
            tfp[u, 1] = tp
    ...
    if ROC:
        ...
        for user in range(model.n_users):
            try:
                if sum(real_per_user[user]) == 0 or sum(
                    real_per_user[user]
                ) == len(real_per_user[user]):
                    continue
                per_user_curves[user] = roc_curve(
                    real_per_user[user], results_per_user[user]
                )
            except:
                pass
        if return_all:
        ...
        while True:
            if not TypeNet:
                (
                    ...
                    src_ips,
                    dst_ips,
                ) = eval_data.get_batch(batch_size=batch_size, device=device)
            else:
                (
                    cont,
                    data,
                    ...
                    users,
                    meta,
                ) = eval_data.typenetbatch(batch_size=batch_size, device=device)
            ...
            if users is None:
                ...
                y_pred.extend(
                    [
                        eval_data.user_from_dim(t)
                        for t in local_y_pred.data.cpu().numpy()
                    ]
                )
...
def load_model(model_type, training_data, args):
    ...
    if args.model_type == "FSNet":
        ...
        model = AuthenticationTransformer(
            training_data.user_count(),
            args.S + args.T,
            args.thead,
            args.tlayer,
            normalizer=normalizer_params,
            partition=(args.P > 0),
            partition_count=args.P,
...
def load_dataset(
    model_type, data_type, data_dir, args, eval_flag=False, reserve=False
):
    model_condition = (
        args.model_type == "classification"
        or args.model_type == "classification_without_others"
    )
    type_condition = args.data_type == "keystrokes_alternate"
    if not model_condition and type_condition:
        data = AlternateAuthenticationKeystrokeDataLoader(
            data_dir,
        ...
        data = AuthenticationKeystrokeDataLoader(
            data_dir,
            run_length=args.N,
            use_ips=args.I,
            evaluation=eval_flag,
        ...
        data = ClassificationKeystrokeDataLoader(
            data_dir,
            run_length=args.N,
            use_ips=args.I,
...
def main():
    ...
    parser.add_argument(
        "data_type", type=str, help="Type of data : generic, keystrokes"
    )
    ...
    parser.add_argument(
        "--detailed-eval", default=0, action="count", help="Detailed evaluation"
    )
    parser.add_argument("-N", type=int, default=512, help="Run length")
    parser.add_argument("-M", type=int, default=5120, help=argparse.SUPPRESS)
    ...
    parser.add_argument(
        "-e", type=int, default=-1, help="Positional encoding dimension"
    )
    ...
    parser.add_argument(
        "--E2", type=int, default=-1, help="Increasing dropout epochs"
    )
    parser.add_argument(
        "--thead", type=int, default=4, help="Transformer heads"
    )
    parser.add_argument(
        "--tlayer", type=int, default=4, help="Transformer layers"
    )
    parser.add_argument(
        "--threshold", type=float, default=0.98, help=argparse.SUPPRESS
    )
    parser.add_argument("--predict", action="count", default=0, help="Predict")
    parser.add_argument(
        "-G", type=float, default=0.95, help="Gamma for LR decay"
    )
    ...
    parser.add_argument(
        "-n", type=int, default=16, help="Min run length (for alternate loader)"
    )
    ...
    training_data = load_dataset(
        args.model_type, args.data_type, train_data_dir, args
    )
    eval_data = load_dataset(
        args.model_type, args.data_type, eval_data_dir, args, eval_flag=True
    )
    model = load_model(args.model_type, training_data, args).to(device)
    n_users = training_data.user_count()
    best_val_loss, best_acc, best_acc_model, best_loss_model = (
        float("inf"),
        0.0,
        None,
        None,
    ...
    print(
        f"You selected {args.model_type} : {args.data_type}. Parameters are : \n\tStride length : {args.N}"
    ...
    if args.E >= 0:
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, args.H, gamma=args.G
        )
        criterion = nn.CrossEntropyLoss(
            weight=training_data.weights(device=device)
        )
        ...
        if epochs > 0:
            for epoch in range(1, epochs + 1):
                ...
                train(
                    ...
                    epoch=epoch - 1,
                    device=device,
                    FSNet=args.model_type == "FSNet",
                    TypeNet=args.model_type == "TypeNet",
                    CNNGRU=args.model_type == "CNNGRU",
                )
                training_data.reset()
                val_loss, acc, weighted_acc, conf_mat, _ = evaluate(
                    model,
                    eval_data,
                    args.B,
                    criterion,
                    device=device,
                    weights=training_data.weights().numpy(),
                    strict=False,
                    FSNet=args.model_type == "FSNet",
                    TypeNet=args.model_type == "TypeNet",
                    CNNGRU=args.model_type == "CNNGRU",
                ...
                scheduler.step()
                if args.data_type == "keystrokes_alternate" and (
                    args.model_type != "classification"
                ):
                    training_data.set_dropout(
                        max(0.0, min(1.0, (epoch - args.E1) / (args.E2)))
                    )
    ...
    for name, data_raw, mod, load in datasets:
        ...
        with open(od + "accuracies", "w") as outfile:
            ...
            for eval_param in vals:
                if (
                    eval_param == -1
                    and args.data_type != "keystrokes_alternate"
                ):
                    print("Error")
                    exit()

                if (
                    args.data_type == "keystrokes_alternate"
                    and eval_param != -1
                ):
                    data.set_dropout(
                        1 - ((eval_param - args.n) / (args.N - args.n))
                    )
                elif eval_param == -1:
                    data.set_dropout(-1)
                ...
                if args.detailed_eval and (args.model_type != "classification"):
                    ...
                    val_loss, acc, weighted_acc, conf_mat, tfp = evaluate(
                        mod,
                        data,
                        args.B,
                        criterion,
                ...
                if args.data_type == "keystrokes":
                    print(
                        f"Using tuples of size {eval_param:3d} | "
                        f"loss {val_loss:5.2f} | ppl {val_ppl:8.2f} | acc {acc} | weighted_acc {weighted_acc}"
                    )
                else:
                    print(
                        f"Using samples of length {eval_param} | "
                        f"loss {val_loss:5.2f} | ppl {val_ppl:8.2f} | acc {acc} | weighted_acc {weighted_acc}"
                    )
                print("-" * 89)
                ...
                if args.model_type != "classification":
                    ...
                    plot_ROC(
                        ...
                        tpr,
                        thsd,
                        od + f"ROC_{eval_param}.png",
                        f"tuples of size {eval_param}"
                        if args.data_type == "keystrokes"
                        else f"samples of length {eval_param}",
                    )
                    for user in range(len(per_user_roc)):
                        if per_user_roc[user] is not None:
                            ...
                            plot_ROC(
                                ...
                                tpr,
                                thsd,
                                od + f"ROC_user{user_lbl}_{eval_param}.png",
                                f"tuples of size {eval_param}"
                                if args.data_type == "keystrokes"
                                else f"samples of length {eval_param}",
                                title=f"user #{user_lbl}",
                            )
                ...
                outfile.write(
                    f"{eval_param}\t{val_loss}\t{acc}\t{weighted_acc}\n"
                )
                if eval_param != -1:
